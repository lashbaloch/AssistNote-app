...
def render_result(result: DetectionResult, threshold: float) -> str:
    ...
    # width="stretch" causes a TypeError in some Streamlit versions.
...
def build_feedback_message(result: DetectionResult, threshold: float) -> str:
    ...
    readable = labels[0]
    readable = readable.replace("_dollar", " dollar")
    readable = readable.replace("AUD", " dollars")
    ...
    return f"{readable} detected."    return st.button("Start Scanning", type="primary")
